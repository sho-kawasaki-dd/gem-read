...
- d:/programming/py_apps/pdf-epub_reader_with_Gemini/src/pdf_epub_reader/dto/document_dto.py — SelectionContent を維持しつつ、複数選択スロットと一覧スナップショットのDTOを追加する。
- d:/programming/py_apps/pdf-epub_reader_with_Gemini/src/pdf_epub_reader/interfaces/view_interfaces.py — IMainView と ISidePanelView に追加選択通知、複数オーバーレイ描画、一覧更新、個別削除、全消去の契約を追加する。
- d:/programming/py_apps/pdf-epub_reader_with_Gemini/src/pdf_epub_reader/presenters/main_presenter.py — 選択世代管理、追加と置換の分岐、非同期完了の差し込み、削除と全消去、文書切替時のクリアを実装する。
- d:/programming/py_apps/pdf-epub_reader_with_Gemini/src/pdf_epub_reader/presenters/panel_presenter.py — 複数選択から連結本文と画像配列を構築し、SidePanel の一覧と連結プレビューを更新する。
- d:/programming/py_apps/pdf-epub_reader_with_Gemini/src/pdf_epub_reader/views/main_window.py — Ctrl 修飾検出、Esc ショートカット、複数ハイライト、番号バッジ、再描画用メソッドを実装する。
- d:/programming/py_apps/pdf-epub_reader_with_Gemini/src/pdf_epub_reader/views/side_panel_view.py — 番号付き一覧、個別削除、全消去、連結プレビュー、読取中表示を実装する。
- d:/programming/py_apps/pdf-epub_reader_with_Gemini/tests/mocks/mock_views.py — 新しい View 契約に対応するモックと simulate helpers を追加する。
- d:/programming/py_apps/pdf-epub_reader_with_Gemini/tests/test_presenters/test_main_presenter.py — 選択状態遷移と非同期順序のテストを追加する。
- d:/programming/py_apps/pdf-epub_reader_with_Gemini/tests/test_presenters/test_panel_presenter.py — 連結本文、画像順序、削除後再計算のテストを追加する。
...
- AI 解析対象は、全選択をユーザーが選んだ順に結合して1回で送る。
- 通常ドラッグは全置換、Ctrl+ドラッグは追加、Esc で全消去する。
- オーバーレイの見た目番号は削除後に 1,2,3... と詰め直すが、内部IDは不変にする。
- SidePanel は 番号付き一覧 + 連結プレビュー を持ち、個別削除と全消去を提供する。
- 選択数は上限なしとし、10件超で注意表示だけ出す。
- 抽出は番号枠を先に確保し、読み取り中表示を出した上で、完了したスロットから中身だけ埋める。

**Further Considerations**

1. 連結フォーマットは、Selection 1 / Page N の見出しを英語にするか、日本語の 選択 1 / ページ N にするかを実装時に UI 言語方針へ揃える。現状は既存UIの日本語に合わせるのが自然。
2. 読み取り失敗スロットは短いエラー表示付きで残す案を推奨する。自動削除にすると、ユーザーが 何が失敗したか を追えなくなるため。
