...
2. Phase 1: clear-all 用 message / command contract を追加する。c:\Users\tohbo\python_programs\gem-read\browser-extension\src\shared\contracts\messages.ts に clear-all runtime message（例: `ClearSelectionBatchMessage`）を追加し、`BackgroundRuntimeMessage` union 型にも同型を追加する（省略すると `entry.ts` ハンドラで型エラーになる）。c:\Users\tohbo\python_programs\gem-read\browser-extension\src\shared\config\phase0.ts と c:\Users\tohbo\python_programs\gem-read\browser-extension\manifest.json に browser command id を追加する。command は Ctrl+Shift+X / Command+Shift+X を割り当てる。
...
5. Phase 2: empty batch auto-create guard を実装する。`openOverlaySession.ts` の `syncArticleCacheState()` に渡す `allowAutoCreate: true` を `session.items.length > 0` に変更する。`runSelectionAnalysis.ts` については変更不要：reuse パス（`resolveAnalysisSession` 内）は `session.items.length > 0` でガード済み、`createFreshSession` 内は呼び出し時点で必ず items が 1 件以上存在するため、いずれも empty batch 状態で呼ばれない。batch が空のときだけ auto-create を抑止し、選択を持つ通常 analyze / rerun は従来どおり維持する。*depends on 1*
6. Phase 2: popup debug cache list API を追加する。c:\Users\tohbo\python_programs\gem-read\src\browser_api\adapters\ai_gateway.py、c:\Users\tohbo\python_programs\gem-read\src\browser_api\application\dto.py、c:\Users\tohbo\python_programs\gem-read\src\browser_api\application\services\analyze_service.py、c:\Users\tohbo\python_programs\gem-read\src\browser_api\api\schemas\cache.py、c:\Users\tohbo\python_programs\gem-read\src\browser_api\api\routers\cache.py に browser-extension prefix 専用の cache list endpoint を追加する。`ai_gateway.py` には `list_all_caches()` メソッドを新規追加し、SDK の `caches.list()` を直接呼ぶ（`AIModel.list_caches()` は `"pdf-reader:"` prefix 専用のため使用しない）。`display_name.startswith("browser-extension:")` によるフィルタリングは `AnalyzeService` 側で行い、`ai_model.py` には手を加えない。popup には browser-extension 所有 cache のみ返す。*parallel with 5*
...
- c:\Users\tohbo\python_programs\gem-read\browser-extension\manifest.json — Ctrl+Shift+X / Command+Shift+X command を定義。
...
- c:\Users\tohbo\python_programs\gem-read\src\browser_api\adapters\ai_gateway.py — `list_all_caches()` メソッドを追加し SDK の `caches.list()` を直接呼ぶ。`AIModel.list_caches()` は `"pdf-reader:"` prefix 専用のため使用しない。
- c:\Users\tohbo\python_programs\gem-read\src\pdf_epub_reader\interfaces\model_interfaces.py — `list_all_caches()` は gateway が SDK を直接呼ぶため Protocol への追加は不要。変更しない。
...
- c:\Users\tohbo\python_programs\gem-read\src\pdf_epub_reader\models\ai_model.py — `list_caches()` が `"pdf-reader:"` prefix 専用であることの確認元。変更しない。
...
**Verification**
1. browser-extension unit tests で以下を追加・更新する: empty batch で overlay reopen しても auto-create されない、選択ありでは従来どおり auto-create 候補判定される、Ctrl+Shift+X command で batch が空になる、Alt+Backspace が editable target 以外でだけ clear-all を送る、popup debug section が list/delete を反映する。
...
- clear-all は browser command と overlay keyboard の両方を提供する。browser command は Ctrl+Shift+X / Command+Shift+X、overlay key は Alt+Backspace。
- Chrome MV3 はショートカット付きコマンドを最大 4 件まで許可する。現在 3 件登録済みのため clear-all 追加後は上限に達する。以降のコマンド追加は不可能になることを承知の上で採用する。
