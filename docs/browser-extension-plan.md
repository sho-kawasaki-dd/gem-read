...
- browser command
    - cached session の再表示: `Ctrl+Shift+8` (`Command+Shift+8`)
    - 現在の選択テキストを batch に追加: `Ctrl+Shift+9` (`Command+Shift+9`)
...
- cached session の再表示は、その tab に cached session があれば full overlay を復元し、無ければ launcher-only overlay を表示する
- `Ctrl+Shift+9` は live な text selection のみを batch に追加し、selection が無い場合は full overlay 上に明示的なエラーを表示する
