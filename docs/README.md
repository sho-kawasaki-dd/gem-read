# Documentation

This directory contains end-user and developer documentation for Gem Read.

## Supported Launch Paths

- Windows PowerShell: `.\gem-read_launch.ps1`
- Canonical module entry point: `uv run python -m pdf_epub_reader`
- Browser extension local API: `uv run python -m browser_api`

The desktop reader remains the primary product. The browser extension docs in this repository describe the current local-first Phase 1 implementation, where the extension talks to a FastAPI process on `127.0.0.1` or `localhost`.

## User Docs

- [docs/user/README.md](docs/user/README.md)
- [docs/user/getting-started.md](docs/user/getting-started.md)
- [docs/user/operations.md](docs/user/operations.md)
- [docs/user/settings-and-cache.md](docs/user/settings-and-cache.md)
- [docs/user/troubleshooting.md](docs/user/troubleshooting.md)

## Developer Docs

- [docs/developer/README.md](docs/developer/README.md)
- [docs/developer/architecture.md](docs/developer/architecture.md)
- [docs/developer/runtime-flows.md](docs/developer/runtime-flows.md)
- [docs/developer/testing.md](docs/developer/testing.md)

## Developer Diagrams

- [docs/developer/diagrams/system-overview.md](docs/developer/diagrams/system-overview.md)
- [docs/developer/diagrams/layer-dependencies.md](docs/developer/diagrams/layer-dependencies.md)
- [docs/developer/diagrams/open-file-sequence.md](docs/developer/diagrams/open-file-sequence.md)
- [docs/developer/diagrams/selection-to-ai-sequence.md](docs/developer/diagrams/selection-to-ai-sequence.md)
- [docs/developer/diagrams/cache-lifecycle.md](docs/developer/diagrams/cache-lifecycle.md)
